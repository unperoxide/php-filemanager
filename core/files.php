...
if (basename(__DIR__) === 'core' && basename(__FILE__) === basename($_SERVER["SCRIPT_FILENAME"])) {
    die('¯\_(ツ)_/');
}
...
//Application Title
define('APP_TITLE', 'Files - ' . @$_SERVER['HTTP_HOST']);
...
$auth_users = isset($auth_users) ? $auth_users : [];
...
$favicon_path = file_exists('favicon.png') ? 'favicon.png' : '';
                                ...
                                <form class="form-signin" action="" method="POST" autocomplete="off">
                                    <div class="form-group">
                                        <div class="text-center py-2">
                                            <img src="<?php echo $favicon_path; ?>" alt="logo" width="40" />
                                        </div>
                                    </div>
                                    <hr />
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="fm_usr" name="fm_usr" autofocus>
                                    </div>
...
if (isset($_POST['ajax']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && $_POST['type'] == "save") {
        ...
        if ($file == '' || !is_file($path . '/' . $file)) {
            ...
            fm_redirect(FM_SELF_URL . '?p=' . fm_urlencode(FM_PATH));
        }
        ...
        <div class="card mb-2 fm-upload-wrapper <?php echo fm_get_theme(); ?>">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link active" href="#fileUploader" data-target="#fileUploader"><span class="material-symbols-rounded">upload_file</span> <?php echo lng('UploadingFiles') ?></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="#urlUploader" class="js-url-upload" data-target="#urlUploader"><span class="material-symbols-rounded">link</span> Upload from URL</a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
            ...
            <div class="card-body">
                <form action="" method="post">
                    ...
                    <p>
                        ...
                        <b><a href="?p=<?php echo fm_urlencode(FM_PATH) ?>" class="btn btn-outline-primary"><i class="fa fa-times-circle"></i> <?php echo lng('Cancel') ?></a></b>
                    </p>
                </form>
            </div>
        </div>
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="path">
        ...
        <p>
            <b><a href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;copy=<?php echo fm_urlencode($copy) ?>&amp;finish=1"><i class="fa fa-check-circle"></i> Copy</a></b> &nbsp;
            <b><a href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;copy=<?php echo fm_urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> Move</a></b> &nbsp;
            <b><a href="?p=<?php echo fm_urlencode(FM_PATH) ?>"><i class="fa fa-times-circle"></i> Cancel</a></b>
        </p>
        ...
        <ul class="folders break-word">
            ...
            if ($parent !== false) {
                ?>
                <li><a href="?p=<?php echo fm_urlencode($parent) ?>&amp;copy=<?php echo fm_urlencode($copy) ?>"><i class="fa fa-chevron-circle-left"></i> ..</a></li>
                <?php
            }
            foreach ($folders as $f) {
                ?>
                <li>
                    <a href="?p=<?php echo fm_urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo fm_urlencode($copy) ?>"><i class="fa fa-folder-o"></i> <?php echo fm_convert_win($f) ?></a></li>
                <?php
            }
            ?>
        </ul>
    ...
    <div class="row">
        <div class="col-12">
            <?php if(!$quickView) { ?>
                ...
                <p class="break-word mb-3">
                    Path: <code><?php echo fm_enc(fm_convert_win($file_path)) ?></code><br>
                    Size: <?php echo ($filesize_raw <= 1000) ? "$filesize_raw bytes" : $filesize; ?><br>
                ...
                <p>
                    <a href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;dl=<?php echo fm_urlencode($file) ?>" class="mr-1 btn btn-sm btn-outline-dark">
                        <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">download</span><?php echo lng('Download') ?>
                    </a>
                    <a href="<?php echo fm_enc($file_url) ?>" target="_blank" class="mr-1 btn btn-sm btn-outline-info">
                        <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">open_in_new</span><?php echo lng('Open') ?>
                    </a>
                    ...
                    if (!FM_READONLY && ($is_zip || $is_gzip) && $filenames !== false) {
                        ...
                        ?>
                        <a href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;unzip=<?php echo fm_urlencode($file) ?>" class="mr-1 btn btn-sm btn-outline-primary">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">expand_content</span><?php echo lng('UnZip') ?>
                        </a>
                        <a href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;unzip=<?php echo fm_urlencode($file) ?>&amp;tofolder=1" title="UnZip to <?php echo fm_enc($zip_name) ?>" class="mr-1 btn btn-sm btn-outline-primary">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">folder_open</span><?php echo lng('UnZipToFolder') ?>
                        </a>
                        <?php
                    }
                    if ($is_text && !FM_READONLY) {
                        ?>
                        <a href="?p=<?php echo fm_urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo fm_urlencode($file) ?>" class="mr-1 btn btn-sm btn-outline-primary">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">edit</span><?php echo lng('Edit') ?>
                        </a>
                        <a href="?p=<?php echo fm_urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo fm_urlencode($file) ?>&env=ace" class="mr-1 btn btn-sm btn-outline-primary">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">code</span><?php echo lng('AdvancedEditor') ?>
                        </a>
                    <?php } ?>
                    <a href="?p=<?php echo fm_urlencode(FM_PATH) ?>" class="mr-1 btn btn-sm btn-outline-danger">
                        <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">arrow_back</span><?php echo lng('Back') ?>
                    </a>
                </p>
                <?php
            }
            if($is_onlineViewer) {
                ...
                if (FM_USE_HIGHLIGHTJS) {
                    ...
                    $content = '<pre class="with-hljs"><code class="p-3 ' . $hljs_class . '">' . fm_enc($content) . '</code></pre>';
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
                <a title="<?php echo lng('Back') ?>" class="mr-1 btn btn-sm btn-outline-danger" href="?p=<?php echo fm_urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo fm_urlencode($file) ?>">
                    <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">arrow_back</span><?php echo lng('Back') ?>
                </a>
                <?php if ($is_text) { ?>
                    <?php if ($isNormalEditor) { ?>
                        <a title="Advanced" class="mr-1 btn btn-sm btn-outline-primary" href="?p=<?php echo fm_urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo fm_urlencode($file) ?>&amp;env=ace">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">code</span><?php echo lng('AdvancedEditor') ?>
                        </a>
                        <button type="button" class="mr-1 btn btn-sm btn-outline-success" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">save</span>Save
                        </button>
                    <?php } else { ?>
                        <a title="Plain Editor" class="mr-1 btn btn-sm btn-outline-primary" href="?p=<?php echo fm_urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo fm_urlencode($file) ?>">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">edit</span><?php echo lng('NormalEditor') ?></a>
                        <button type="button" class="mr-1 btn btn-sm btn-outline-success" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')">
                            <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">save</span><?php echo lng('Save') ?>
                        </button>
                    <?php } ?>
                <?php } ?>
                <a title="<?php echo lng('BackUp') ?>" class="d-none mr-1 btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="backup('<?php echo fm_urlencode(trim(FM_PATH)) ?>','<?php echo fm_urlencode($file) ?>')">
                    <span class="material-symbols-rounded align-text-top mr-1" style="font-size: 18px;">backup</span><?php echo lng('BackUp') ?>
                </a>
            </div>
        </div>
        <?php
        if ($is_text && $isNormalEditor) {
            echo '<textarea class="w-full p-3 form-control form-control-sm text-monospace" style="margin-top: 1.25rem;" id="normal-editor" rows="33" cols="120">' . htmlspecialchars($content) . '</textarea>';
        } elseif ($is_text) {
            echo '<div id="editor" contenteditable="true" style="margin-top: 2.75rem;">' . htmlspecialchars($content) . '</div>';
        } else {
...
<form action="" method="post">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover _table-sm <?php echo $tableTheme; ?>" id="main-table">
            ...
            if ($parent !== false) {
                ...
                <tr><?php if (!FM_READONLY): ?>
                    ...
                    <td class="border-0">
                        <a href="?p=<?php echo fm_urlencode($parent) ?>"><span class="material-symbols-rounded">arrow_back</span></a>
                    </td>
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td>
                        <div class="filename"><a href="?p=<?php echo fm_urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win(fm_enc($f)) ?>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td><?php if (!FM_READONLY): ?><a title="Change Permissions" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;chmod=<?php echo fm_urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
                        <td><?php echo $owner['name'] . ':' . $group['name'] ?></td>
                    <?php endif; ?>
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete')?>" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;del=<?php echo fm_urlencode($f) ?>" onclick="return confirm('<?php echo lng('Delete').' '.lng('Folder').'?'; ?>\n \n ( <?php echo fm_urlencode($f) ?> )');"><span class="material-symbols-rounded">delete</span></a>
                            <a title="<?php echo lng('Rename')?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><span class="material-symbols-rounded">edit</span></i></a>
                            <a title="<?php echo lng('CopyTo')?>..." href="?p=&amp;copy=<?php echo fm_urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><span class="material-symbols-rounded">file_copy</span></a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink')?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><span class="material-symbols-rounded">link</span></a>
                    </td>
                </tr>
                <?php
                flush();
            ...
            foreach ($files as $f) {
                ...
                $filesize = fm_get_filesize($filesize_raw);
                $filelink = '?p=' . fm_urlencode(FM_PATH) . '&amp;view=' . fm_urlencode($f);
                ...
                <tr>
                    ...
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td><?php if (!FM_READONLY): ?><a title="<?php echo 'Change Permissions' ?>" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;chmod=<?php echo fm_urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?>
                        </td>
                        <td><?php echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
                    <?php endif; ?>
                    <td class="inline-actions">
                        <a title="<?php echo lng('Preview') ?>" href="<?php echo $filelink.'&quickView=1'; ?>" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php echo fm_convert_win(fm_enc($f)) ?>" data-max-width="100%" data-width="100%"><span class="material-symbols-rounded">visibility</span></a>
                        <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Delete') ?>" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;del=<?php echo fm_urlencode($f) ?>" onclick="return confirm('<?php echo lng('Delete').' '.lng('File').'?'; ?>\n \n ( <?php echo fm_urlencode($f) ?> )');"><span class="material-symbols-rounded">delete</span></a>
                            <a title="<?php echo lng('Rename') ?>" href="#" onclick="rename('<?php echo fm_enc(addslashes(FM_PATH)) ?>', '<?php echo fm_enc(addslashes($f)) ?>');return false;"><span class="material-symbols-rounded">edit</span></a>
                            <a title="<?php echo lng('CopyTo') ?>..." href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;copy=<?php echo fm_urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">
                                <span class="material-symbols-rounded">file_copy</span>
                            </a>
                            <a title="Edit" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;edit=<?php echo fm_urlencode($f) ?>&amp;env=ace">
                                <span class="material-symbols-rounded">code</span>
                            </a>
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink') ?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><span class="material-symbols-rounded small">link</span></a>
                        <a title="<?php echo lng('Download') ?>" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;dl=<?php echo fm_urlencode($f) ?>"><span class="material-symbols-rounded">download</span></a>
                    </td>
                </tr>
                <?php
                flush();
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-9">
            <ul class="list-inline footer-action">
                <li class="list-inline-item"> <a href="#/select-all" class="btn btn-sm btn-outline-primary" onclick="select_all();return false;"><i class="fa fa-check-square"></i> <?php echo lng('SelectAll') ?> </a></li>
                <li class="list-inline-item"><a href="#/unselect-all" class="btn btn-sm btn-outline-primary" onclick="unselect_all();return false;"><i class="fa fa-window-close"></i> <?php echo lng('UnSelectAll') ?> </a></li>
                <li class="list-inline-item"><a href="#/invert-all" class="btn btn-sm btn-outline-primary" onclick="invert_all();return false;"><i class="fa fa-th-list"></i> <?php echo lng('InvertSelection') ?> </a></li>
                <li class="list-inline-item"><input type="submit" class="hidden" name="delete" id="a-delete" value="Delete" onclick="return confirm('<?php echo lng('Delete selected files and folders?'); ?>')">
                    <a href="javascript:document.getElementById('a-delete').click();" class="btn btn-sm btn-outline-primary"><i class="fa fa-trash"></i> <?php echo lng('Delete') ?> </a></li>
                <li class="list-inline-item"><input type="submit" class="hidden" name="zip" id="a-zip" value="zip" onclick="return confirm('<?php echo lng('Create archive?'); ?>')">
                    <a href="javascript:document.getElementById('a-zip').click();" class="btn btn-sm btn-outline-primary"><i class="fa fa-file-archive-o"></i> <?php echo lng('Zip') ?> </a></li>
                <li class="list-inline-item"><input type="submit" class="hidden" name="tar" id="a-tar" value="tar" onclick="return confirm('<?php echo lng('Create archive?'); ?>')">
                    <a href="javascript:document.getElementById('a-tar').click();" class="btn btn-sm btn-outline-primary"><i class="fa fa-file-archive-o"></i> <?php echo lng('Tar') ?> </a></li>
                <li class="list-inline-item"><input type="submit" class="hidden" name="copy" id="a-copy" value="Copy">
                    <a href="javascript:document.getElementById('a-copy').click();" class="btn btn-sm btn-outline-primary"><i class="fa fa-files-o"></i> <?php echo lng('Copy') ?> </a></li>
            </ul>
        </div>
        <?php endif; ?>
    </div>
...
function fm_set_msg($msg, $status = 'success')
{
    ...
    $_SESSION[FM_SESSION_ID]['status'] = $status === 'error' ? 'danger' : 'success';
}
...
function fm_show_nav_path($path)
{
    ...
    ?>
    <nav class="navbar navbar-expand-lg py-3 shadow-sm <?php echo $getTheme; ?> mb-4 main-nav <?php echo $isStickyNavBar ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            $sep = '<span class="bread-crumb material-symbols-rounded px-1" style="font-size: 18px;">chevron_right</span>';
            ...
            if ($path != '') {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $parent_enc = fm_urlencode($parent);
                ...
                <ul class="navbar-nav mr-auto float-right <?php echo fm_get_theme();  ?>">
                    <li class="nav-item px-2">
                        <div class="input-group">
                            <input type="text" class="form-control w-100" placeholder="<?php echo lng('Search') ?>" aria-label="<?php echo lng('Search') ?>" aria-describedby="search-addon2" id="search-addon">
                    ...
                    <?php if (!FM_READONLY): ?>
                    <li class="nav-item px-2">
                        <a title="<?php echo lng('Upload') ?>" class="btn btn-primary _btn-sm" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;upload"><i class="mr-1 fa fa-cloud-upload" aria-hidden="true"></i> <?php echo lng('Upload') ?></a>
                    </li>
                    <li class="nav-item px-2">
                        <a title="<?php echo lng('NewItem') ?>" class="btn btn-success _btn-sm" href="#createNewItem" data-toggle="modal" data-target="#createNewItem"><i class="mr-1 fa fa-plus-square"></i> <?php echo lng('NewItem') ?></a>
                    </li>
                    <?php endif; ?>
                    <?php if (FM_USE_AUTH): ?>
                    <li class="nav-item avatar dropdown">
                        <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-5" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php if(isset($_SESSION[FM_SESSION_ID]['logged'])) { echo $_SESSION[FM_SESSION_ID]['logged']; } ?> </a>
                        <div class="dropdown-menu dropdown-menu-right <?php echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                            <?php if (!FM_READONLY): ?>
                            <a title="<?php echo lng('Settings') ?>" class="dropdown-item" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;settings=1"><?php echo lng('Settings') ?></a>
                            <?php endif ?>
                            <a title="<?php echo lng('Help') ?>" class="dropdown-item" href="?p=<?php echo fm_urlencode(FM_PATH) ?>&amp;help=2"><?php echo lng('Help') ?></a>
                            <a title="<?php echo lng('Logout') ?>" class="dropdown-item" href="?logout=1"><?php echo lng('Logout') ?></a>
                        </div>
                    </li>
                    <?php else: ?>
                        <?php if (!FM_READONLY): ?>
...
function fm_show_message()
{
    if (isset($_SESSION[FM_SESSION_ID]['message'])) {
        $class = isset($_SESSION[FM_SESSION_ID]['status']) ? $_SESSION[FM_SESSION_ID]['status'] : 'success';
        echo '<p class="alert my-1 mb-3 alert-' . $class . '">' . $_SESSION[FM_SESSION_ID]['message'] . '</p>';
...
<head>
    ...
    <style>
        ...
        .inline-actions a .material-symbols-rounded {
            margin-right: 12px;
        }
        ...
        .fa.fa-home { font-size:1.5rem;vertical-align: -2px  }
        ...
        .inline-actions > a > i { font-size:1em;margin-left:8px;padding:6px;border-radius:50px !importanr  }
        ...
        #snackbar { visibility:hidden;min-width:250px;margin-left:-125px;background-color:#333;color:#fff;text-align:center;border-radius:4px;padding:16px;position:fixed;z-index:1;left:50%;bottom:30px;font-size:17px; }
...
<script>
    ...
    function backup(e, t) { //Create file backup with .bck
        ...
        return n.open("POST", "", !0), n.setRequestHeader("Content-type", "application/x-www-form-fm_urlencoded"), n.onreadystatechange = function () {
...
<?php if (isset($_GET['edit']) && isset($_GET['env']) && FM_EDIT_FILE):
    ...
    <script>
        ...
        const ACE_EDITOR_FONT_SIZE = 14;

        editor.setOptions({
            fontFamily: "SF Mono, Roboto Mono, JetBrains Mono, Menlo, Consolas, monospace",
            fontSize: `${ACE_EDITOR_FONT_SIZE}px`
        });
        ...
        function renderThemeMode() {
            ...
            $fontSizeEl.val(ACE_EDITOR_FONT_SIZE).change(); //set default font size in drop down
        }
